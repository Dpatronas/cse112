...
(define (eval-expr expr)
    ...
    (cond ((number? expr) (+ expr 0.0))
          ...
          ((pair? expr)
                ...
                (cond ((equal? func "asub") ;; cond
                	;; TRUE: Get the array using the name = key
       	        	;; Check if the array index (caddr expr) is a symbol/var
       	        		(cond
					((symbol? (caddr expr))
                        			(let ((array (hash-ref *array-table* (cadr expr) #f))
                                			(index (exact-round (hash-ref *var-table* (caddr expr) 0.0))))
                                			;;(printf "~a" (vector-ref array index))))
                                			(eval-expr (vector-ref array index))))
	             			(else
                                		(let ((array (hash-ref *array-table* (cadr expr) #f))
...
(define (interp-let args continuation)
    (cond ((not (list? (car args)))
          ...
          (else
               (cond ((symbol? (caddar args))
                          (let ((array (hash-ref *array-table* (cadar args) #f))
                               (index (exact-round (hash-ref *var-table* (caddar args) 0.0)))
                               (set   (eval-expr (cadr args))))
                               (vector-set! array (exact-round (eval-expr index)) (eval-expr set))))
                     (else
                          (let ((array (hash-ref *array-table* (cadar args) #f))
                               (index (caddar args))
                               (set   (eval-expr (cadr args))))
...
(define (interp-input args continuation)
    (define(hash-var var)
           (let((value (read)))
		 (cond ((eof-object? value) (hash-set! *var-table* 'eof 1.0))
		       (else (hash-set! *var-table* var value)))))
    (map hash-var args)
    (interp-program continuation))
